...
class nosound():
    def __init__(self):
        ...
        if not shutil.which('apt'):
            print('this section is only made for distro based on apt package manager :(.')
...
def uperror():
    ...
    try:
        ...
        print("i'm sorry, this section was only for distro based on apt package manager :( ")
        time.sleep(2)
        print('\nplease submitt issue in this link- https://github.com/SACHIT69/Dr.fixit/issues ||Any feedback would be greatly appreciated. Thank you')
        time.sleep(3)
...
def update():
    ...
    time.sleep(1.5)
    try:
        os.system('sudo apt update -y && sudo apt full-upgrade -y && sudo apt dist-upgrade -y')
    except os.error:
